...
/** Port du front public, où mène un lien de vérification. */
const PORT_PUBLIC = import.meta.env.VITE_PUBLIC_PORT || '5174';

/**
 * Adresse du front public, déduite elle aussi de la page courante.
 *
 * Elle sert à composer le lien qu'un diplômé transmet à un employeur.
 * Figée au build, elle survivait à un changement de réseau et proposait
 * un lien mort — le pire des cas, puisqu'il est fait pour être partagé.
 */
export function adressePublique() {
  const explicite = import.meta.env.VITE_PUBLIC_URL;
  if (explicite) return explicite;
  if (typeof window === 'undefined' || !window.location?.hostname) {
    return `http://localhost:${PORT_PUBLIC}`;
  }
  const { protocol, hostname } = window.location;
  return `${protocol}//${hostname}:${PORT_PUBLIC}`;
}
