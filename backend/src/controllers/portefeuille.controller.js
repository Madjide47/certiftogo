...
export async function listerDiplomes(req, res, next) {
  try {
    const diplomes = await portefeuilleService.lister(req.utilisateur.personne_id);
...
export async function statistiques(req, res, next) {
  try {
    const stats = await portefeuilleService.statistiques(req.utilisateur.personne_id);
